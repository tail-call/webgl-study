...
void main() {
    ...
    vColor = aVertexColor * gl_Position;
}
`;
    }
...
`;
    ...
    loadTexture(url) {
        ...
        gl.texImage2D(gl.TEXTURE_2D, mipmapLevel, internalFormat, width, height, border, srcFormat, srcType, pixel);

        const image = new Image();
        image.onload = () => {
            gl.bindTexture(gl.TEXTURE_2D, texture);
            gl.texImage2D(gl.TEXTURE_2D, mipmapLevel, internalFormat, srcFormat, srcType, image);
